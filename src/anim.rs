...
impl Animation<Rect> {
	...
	/// equal-size tiles.
	pub fn from_sheet(bounds: &Rect, row_top: i32, tile_width: u32, tile_height: u32, frame_length: Duration, base: Instant) -> Animation<Rect> {
	...
	/// spritesheet.
	pub fn n_from_sheet(row_top: i32, tile_width: u32, tile_height: u32, n: u32, frame_length: Duration, base: Instant) -> Animation<Rect> {
		let mut v = Vec::with_capacity(n as usize);
		for i in 0..n {
			v.push(Rect::new((i*tile_width) as i32, row_top, tile_width, tile_height));
		}
...
mod tests {
	...
	fn tick_at_advances() {
		...
		let cases = vec![
			Testcase{when: d - Duration::from_micros(1), want: 0},
			Testcase{when: d, want: 1},
			Testcase{when: 2*d, want: 2},
			Testcase{when: 3*d, want: 0},
			Testcase{when: 6*d, want: 0},
			Testcase{when: 7*d, want: 1},
			Testcase{when: 60*d, want: 0},
		];
		for c in cases {
			let t = start + c.when;
			println!("d={:?} t={:?} frame={:?} next={:?}", c.when, t, anim.k, anim.next_frame);
			let r = *anim.tick_at(t);
			assert_eq!(r, v[c.want], "wrong tick: d is {:?}, got {}, want {}", c.when, r, v[c.want]);
